    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-2 sm:p-4'>
      ...
      <div className='bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] flex flex-col overflow-hidden'>
        ...
        <div className='flex justify-between items-center px-4 py-3 sm:px-6 sm:py-4 border-b border-slate-100 bg-slate-50 flex-shrink-0'>
          <div className='flex items-center gap-3'>
            <div className='p-2 bg-blue-600 rounded-lg text-white'>
              <SiBmw size={24} className='sm:size-[30px] text-slate-950' />
            </div>
            ...
            <div className='space-y-4'>
              ...
              <div className='grid grid-cols-1 gap-2'>
                {[
                  ...
                  >
                    <span className='text-[10px] sm:text-xs font-bold text-slate-500 uppercase pr-2'>
                      {item.label}
                    </span>
                    <span className='text-xs sm:text-sm font-mono font-bold text-blue-700 break-all text-right'>
                      {item.value || "—"}
                    </span>
                  </div>
                ))}
        ...
        <div className='px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end flex-shrink-0'>
          <button
            onClick={onClose}
            className='w-full sm:w-auto px-6 py-2 bg-slate-800 text-white rounded-xl hover:bg-slate-700 transition-colors shadow-lg'
          >
            Закрыть
          </button>
        </div>
